defmodule DynamicForm.Changeset do
  ...
  If you have JSON or a map, parse it to an Instance struct first:
      ...
      # Parse at the edge
      instance = DynamicForm.Parser.FromData.parse!(json_or_map)
  ...
  def create_changeset(%Instance{} = instance, params \\ %{}, opts \\ []) do
    ...
    decoded_params =
      ...
      |> NestedForms.normalize_params(questions)
    ...
    # Entry changesets inherit the form-level questions so a carried-forward
    # source declared at the top level resolves from inside a template.
    opts = Keyword.put_new(opts, :root_questions, questions)
  ...
  # NestedForms.validate/3 enforces required-ness for them instead.
